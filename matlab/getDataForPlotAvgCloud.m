function [avgTrT, stdT, goodSpT, timestepT, NPointsWinNegLocalT, ...
    ...
    ampLSTD, ampRSTD, ampL, ampR, ...
          avgAmpL, avgAmpR, stdAmpL, stdAmpR] = getDataForPlotAvgCloud(traces, ...
...
% Standard deviations
ampLSTD = sum(stdT([minNegIPSCNNT; maxIPSCNNT]))
ampRSTD = sum(stdT([minPosIPSCNNT; maxIPSCNNT]))

%%%%%%%%%%% Cengiz's STDs => They're useless without spike-triggered averaging
