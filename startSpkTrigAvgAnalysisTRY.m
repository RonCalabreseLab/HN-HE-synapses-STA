...
function startSpkTrigAvgAnalysisTRY
...
% assumes correct parsing of file name conform convention
[namesHN, ~, flagNew] = getHNnames(file_nameInp) % was heName
[~,c] = size(namesHN);

a = size(origTraces);
noChanels = a(1,2); %%% default, flag = 0, initial format [time pre pre post]
clear a;
...
if ~isempty(answer) % quit program    
    ...
    if ck > 0
        ...
        if flagNew == 0    %%% initial format [time pre pre post]    
            data = [origTraces(:,1) origTraces(:,ck+1) origTraces(:,noChanels)];
        end
        if flagNew == 1   %%% new format [time pre post time pre post] 
            idx = (ck - 1) * 3 + 1
            
            %%% eliminate the 0s
            [~,~,auxTime] = find(origTraces(:, idx));
            [~,~,auxPre] = find(origTraces(:, idx + 1));
            [~,~,auxPost] = find(origTraces(:, idx + 2));
            data = [auxTime auxPre auxPost];
            
            size(data)
        end
%         sta_5_master_sta(data, strOfTop10Lines, file_nameInp, ...
%             namesHN{ck}); % starts analysis
        sta_5_masterTRY(data, strOfTop10Lines, file_nameInp, ...
            namesHN{ck}); % starts analysis
        clear data;
    else
